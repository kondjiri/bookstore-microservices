...
import com.fanedesign.service.VenteService;
...
/**
 * REST controller for managing {@link com.fanedesign.domain.Vente}.
 */
@RestController
@RequestMapping("/api")
public class VenteResource {
    ...
    public ResponseEntity<Vente> partialUpdateVente(@PathVariable(value = "id", required = false) final Long id, @RequestBody Vente vente)
        ...
        Optional<Vente> result = venteService.partialUpdate(vente);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, vente.getId().toString())
        );
    }
    ...
    public ResponseEntity<Vente> getVente(@PathVariable Long id) {
        ...
        Optional<Vente> vente = venteService.findOne(id);
